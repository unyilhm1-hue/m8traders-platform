...
export function TradingChart({ data, onPriceChange, className = '' }: TradingChartProps) {
    ...
    useEffect(() => {
        ...
        const chart: any = init(chartRef.current, {
            styles: chartTheme,
            // ✅ FIX: Configure bar spacing to prevent "renggang" (sparse) candles
            customApi: {
                formatDate: (dateTimeFormat: any, timestamp: number, format: string) => {
        ...
        if (chart) {
            ...
            // ✅ CRITICAL: Set bar space config AFTER initialization
            // This prevents candles from being too sparse (renggang)
            try {
                chart.setBarSpace(6); // Optimal spacing for readability (default is often 3-4)
                chart.setRightMinVisibleBarCount(50); // Minimum bars visible on right
                chart.setLeftMinVisibleBarCount(5); // Minimum bars visible on left
                console.log('[TradingChart] Bar spacing configured: 6px per bar');
            } catch (e) {
                console.warn('[TradingChart] Could not set bar space config:', e);
            }
    ...
    useEffect(() => {
        ...
        // Show initial candles (limit to last 100 to prevent barcode effect while maintaining good spacing)
        // This ensures chart starts zoomed-in with readable candles
        const visibleData = engine.getVisibleData();
        const maxInitialCandles = 100; // ✅ Reduced from 150 to improve spacing
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Method 2: Explicitly remove ALL indicator types from their panes
            // KLineChart v9 workaround - global removeIndicator() doesn't always clear panes
            // We need to explicitly remove each indicator type from its specific pane

            // Get all unique indicator types that might have been enabled before
            const allIndicatorTypes = Object.keys(INDICATOR_CONFIG) as IndicatorType[];

            allIndicatorTypes.forEach((type) => {
                const config = INDICATOR_CONFIG[type];
                if (!config) return;

                try {
                    if (config.pane === 'main') {
                        // Main pane indicators (overlays) - remove from candle_pane
                        chart.removeIndicator('candle_pane', config.name);
                        console.log(`✅ Explicitly removed ${config.name} from candle_pane`);
                    } else {
                        // Sub-pane indicators - remove from their dedicated pane
                        const paneId = `${type}-pane`;
                        chart.removeIndicator(paneId, config.name);
                        console.log(`✅ Explicitly removed ${config.name} from ${paneId}`);
                    }
                } catch (e) {
                    // Silent fail - indicator might not exist, which is fine
                    // Only log if verbose debugging needed
                    // console.warn(`Could not remove ${config.name}:`, e);
                }
            });
